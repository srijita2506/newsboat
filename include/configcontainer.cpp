	...
	"[ConfigContainer]")
{
	...
	SECTION("unreadfirst") {
		cfg.set_configvalue("article-sort-order", "unreadfirst");
		sort_strategy = cfg.get_article_sort_strategy();
		REQUIRE(sort_strategy.sm == ArtSortMethod::UNREAD_FIRST);
		REQUIRE(sort_strategy.sd == SortDirection::DESC);

		cfg.set_configvalue("article-sort-order", "unreadfirst-asc");
		sort_strategy = cfg.get_article_sort_strategy();
		REQUIRE(sort_strategy.sm == ArtSortMethod::UNREAD_FIRST);
		REQUIRE(sort_strategy.sd == SortDirection::ASC);

		cfg.set_configvalue("article-sort-order", "unreadfirst-desc");
		sort_strategy = cfg.get_article_sort_strategy();
		REQUIRE(sort_strategy.sm == ArtSortMethod::UNREAD_FIRST);
		REQUIRE(sort_strategy.sd == SortDirection::DESC);
	}
